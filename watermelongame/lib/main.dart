...
class _GamePageState extends State<GamePage> {
  ...
  void _openLobbyDialog() {
    showDialog(
        ...
        builder: (context) {
          return _LobbyDialog(
            onGameStartPressed: (gameId) async {
              // Add duration to allow subscribing to a new channel in a realtime callback
              await Future.delayed(Duration.zero);
...
class _LobbyDialog extends StatefulWidget {
  const _LobbyDialog({
    required this.onGameStartPressed,
  });

  final void Function(String gameId) onGameStartPressed;
  ...
  void initState() {
    ...
    _lobbyChannel.on(RealtimeListenTypes.presence, ChannelFilter(event: 'sync'),
      ...
      if (participantIds.contains(myUserId)) {
        ...
        widget.onGameStartPressed(gameId);
        Navigator.of(context).pop();
      }
    }).subscribe(
      (status, [ref]) async {
